...
rm -rf $HOME/src/c-blosc2-cpu-build
cmake -S $HOME/src/c-blosc2 -B $HOME/src/c-blosc2-cpu-build -DBUILD_TESTS=OFF -DBUILD_BENCHMARKS=OFF -DDEACTIVATE_AVX2=OFF -DCMAKE_INSTALL_PREFIX=${SW_DIR}/c-blosc2-2.11.1
cmake --build $HOME/src/c-blosc2-cpu-build --target install --parallel 16
rm -rf $HOME/src/c-blosc2-cpu-build
...
cmake -S $HOME/src/adios2 -B $HOME/src/adios2-cpu-build -DADIOS2_USE_Blosc=ON -DADIOS2_USE_HDF5=OFF -DADIOS2_USE_Fortran=OFF -DADIOS2_USE_Python=OFF -DADIOS2_USE_ZeroMQ=OFF -DCMAKE_INSTALL_PREFIX=${SW_DIR}/adios2-2.9.2
