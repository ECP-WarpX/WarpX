...
if arch == "GPU":
    text = re.sub(
        "addToCompileString =", "addToCompileString = USE_GPU=TRUE USE_OMP=FALSE ", text
    )
...
if ci_openpmd:
    text = re.sub(
        "addToCompileString =", "addToCompileString = USE_OPENPMD=TRUE ", text
    )

# always build with PSATD support (runtime controlled if used)
if ci_psatd:
    text = re.sub("addToCompileString =", "addToCompileString = USE_FFT=TRUE ", text)
    text = re.sub("USE_FFT=FALSE", "", text)

# CCache
if ci_ccache:
    text = re.sub("addToCompileString =", "addToCompileString = USE_CCACHE=TRUE ", text)
...
if ci_num_make_jobs is not None:
    text = re.sub(
        "numMakeJobs = \d+", "numMakeJobs = {}".format(ci_num_make_jobs), text
    )
...
if ci_regular_cartesian_1d:
    ...
    test_blocks = select_tests(
        test_blocks, ["PRECISION=FLOAT", "USE_SINGLE_PRECISION_PARTICLES=TRUE"], False
    )
...
if ci_single_precision:
    test_blocks = select_tests(
        test_blocks, ["PRECISION=FLOAT", "USE_SINGLE_PRECISION_PARTICLES=TRUE"], True
    )
