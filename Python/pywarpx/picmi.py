...
class Simulation(picmistandard.PICMI_Simulation):
    ...
    warpx_do_psatd_JRm: bool, default=0
        Whether to use the PSATD-JRm algorithm, where current deposition and
        field update are performed multiple times within each time step.

    warpx_do_psatd_JRm_n_depositions: integer
        Number of sub-steps to use with the PSATD-JRm algorithm, when ``warpx_do_psatd_JRm=1``.
        Note that this input parameter is not optional and must always be set in all
        input files where ``warpx.do_psatd_JRm=1``. No default value is provided automatically.
    ...
    def init(self, kw):
        ...
        self.do_psatd_JRm = kw.pop('warpx_do_psatd_JRm', None)
        self.do_psatd_JRm_n_depositions = kw.pop('warpx_do_psatd_JRm_n_depositions', None)
    ...
    def initialize_inputs(self):
        ...
        pywarpx.warpx.do_psatd_JRm = self.do_psatd_JRm
        pywarpx.warpx.do_psatd_JRm_n_depositions = self.do_psatd_JRm_n_depositions
