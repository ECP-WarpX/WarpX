...
{
    WX_PROFILE("PhysicalParticleContainer::AddParticles()");
...
{
    WX_PROFILE("PhysicalParticleContainer::AddPlasma");
...
{
    WX_PROFILE("PPC::Evolve()");
    WX_PROFILE_VAR_NS("PPC::Evolve::Copy", blp_copy);
    WX_PROFILE_VAR_NS("PPC::FieldGather", blp_fg);
    WX_PROFILE_VAR_NS("PPC::EvolveOpticalDepth", blp_ppc_qed_ev);
    WX_PROFILE_VAR_NS("PPC::ParticlePush", blp_ppc_pp);
...
#endif
    {
        ...
        for (WarpXParIter pti(*this, lev); pti.isValid(); ++pti)
        {
            ...
            if (! do_not_push)
            {
                ...
                WX_PROFILE_VAR_STOP(blp_fg);

#ifdef WARPX_QED
                //
                //Evolve Optical Depth
                //
                WX_PROFILE_VAR_START(blp_ppc_qed_ev);
                EvolveOpticalDepth(pti, dt);
                WX_PROFILE_VAR_STOP(blp_ppc_qed_ev);
#endif

                //
                // Particle Push
                //
                WX_PROFILE_VAR_START(blp_ppc_pp);
                PushPX(pti, dt, a_dt_type);
                WX_PROFILE_VAR_STOP(blp_ppc_pp);
...
PhysicalParticleContainer::PushP (int lev, Real dt,
                                  ...
                                  const MultiFab& Bx, const MultiFab& By, const MultiFab& Bz)
{
    WX_PROFILE("PhysicalParticleContainer::PushP");
...
{
    WX_PROFILE("PhysicalParticleContainer::GetParticleSlice");
...
{
    WX_PROFILE("PPC::getIonizationFunc");
