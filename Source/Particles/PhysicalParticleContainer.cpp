        ...
        for (WarpXParIter pti(*this, lev); pti.isValid(); ++pti)
        {
            ...
            if (! do_not_push)
            {
                ...
                if (WarpX::do_electrostatic == ElectrostaticSolverAlgo::None) {
                    ...
                    DepositCurrent(pti, wp, uxp, uyp, uzp, ion_lev, &jx, &jy, &jz, icomp,
                                   0, np_current, thread_num, lev, lev, dt, -0.5_rt); // Deposit current at t_{n+1/2}
                    if (has_buffer){
                        // Deposit in buffers
                        DepositCurrent(pti, wp, uxp, uyp, uzp, ion_lev, cjx, cjy, cjz,
                                       0, np_current, np-np_current, thread_num, lev, lev-1, dt, -0.5_rt); // Deposit current at t_{n+1/2}
                    }
