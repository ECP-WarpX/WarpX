...
FullDiagnostics::AddRZModesToDiags (int lev)
{
    ...
    if (divE_requested) {
        ...
        icomp += 1;
        if (m_format == "openpmd") {
            m_varnames.erase(std::remove(m_varnames.begin(), m_varnames.end(), "divE"), m_varnames.end());
        }
    ...
    if (rho_requested) {
        ...
        icomp += 1;
        if (m_format == "openpmd") {
            m_varnames.erase(std::remove(m_varnames.begin(), m_varnames.end(), "rho"), m_varnames.end());
        }
