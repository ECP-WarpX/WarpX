...
FullDiagnostics::InitializeFieldFunctorsRZopenPMD (int lev)
{
    ...
    for (int dim=0; dim<3; dim++){
        AMREX_ALWAYS_ASSERT(
            warpx.m_fields.get(FieldType::Efield_aux, Dir{dim}, lev)->nComp() == ncomp_multimodefab );
        AMREX_ALWAYS_ASSERT(
            warpx.m_fields.get(FieldType::Bfield_aux, Dir{dim}, lev)->nComp() == ncomp_multimodefab );
        AMREX_ALWAYS_ASSERT(
            warpx.m_fields.get(FieldType::current_fp, Dir{dim}, lev)->nComp() == ncomp_multimodefab );
    }
    ...
    for (int comp=0; comp<m_varname_fields_size; comp++){
        if        ( m_varnames_fields[comp] == "Er" ){
            m_all_field_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Efield_aux, 0_dir, lev), lev, m_crse_ratio,
                                                        false, ncomp);
            if (update_varnames) {
                AddRZModesToOutputNames(std::string("Er"), ncomp);
            }
        } else if ( m_varnames_fields[comp] == "Et" ){
            m_all_field_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Efield_aux, 1_dir, lev), lev, m_crse_ratio,
                                                        false, ncomp);
            if (update_varnames) {
                AddRZModesToOutputNames(std::string("Et"), ncomp);
            }
        } else if ( m_varnames_fields[comp] == "Ez" ){
            m_all_field_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Efield_aux, 2_dir, lev), lev, m_crse_ratio,
                                                        false, ncomp);
            if (update_varnames) {
                AddRZModesToOutputNames(std::string("Ez"), ncomp);
            }
        } else if ( m_varnames_fields[comp] == "Br" ){
            m_all_field_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Bfield_aux, 0_dir, lev), lev, m_crse_ratio,
                                                        false, ncomp);
            if (update_varnames) {
                AddRZModesToOutputNames(std::string("Br"), ncomp);
            }
        } else if ( m_varnames_fields[comp] == "Bt" ){
            m_all_field_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Bfield_aux, 1_dir, lev), lev, m_crse_ratio,
                                                        false, ncomp);
            if (update_varnames) {
                AddRZModesToOutputNames(std::string("Bt"), ncomp);
            }
        } else if ( m_varnames_fields[comp] == "Bz" ){
            m_all_field_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Bfield_aux, 2_dir, lev), lev, m_crse_ratio,
                                                        false, ncomp);
            if (update_varnames) {
...
FullDiagnostics::AddRZModesToDiags (int lev)
{
    ...
    for (int dim=0; dim<3; dim++){
        ...
        m_all_field_functors[lev].push_back(std::make_unique<CellCenterFunctor>(
                warpx.m_fields.get(FieldType::Efield_aux, Dir{dim}, lev), lev,
                    m_crse_ratio, false, ncomp_multimodefab));
        AddRZModesToOutputNames(std::string("E") + coord[dim],
                warpx.m_fields.get(FieldType::Efield_aux, 0_dir, 0)->nComp());
    }
    // B
    for (int dim=0; dim<3; dim++){
        // 3 components, r theta z
        m_all_field_functors[lev].push_back(std::make_unique<CellCenterFunctor>(
                warpx.m_fields.get(FieldType::Bfield_aux, Dir{dim}, lev), lev,
                    m_crse_ratio, false, ncomp_multimodefab));
        AddRZModesToOutputNames(std::string("B") + coord[dim],
                warpx.m_fields.get(FieldType::Bfield_aux, 0_dir, 0)->nComp());
    }
    // j
    for (int dim=0; dim<3; dim++){
        // 3 components, r theta z
        ...
        AddRZModesToOutputNames(std::string("J") + coord[dim],
                warpx.m_fields.get(FieldType::current_fp,0_dir,0)->nComp());
    }
    // divE
    if (divE_requested) {
...
FullDiagnostics::InitializeFieldFunctors (int lev)
{
    ...
    for (int comp=0; comp<nvar; comp++){
        if        ( m_varnames[comp] == "Ez" ){
            ...
            m_all_field_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::vector_potential_fp_nodal, 2_dir, lev), lev, m_crse_ratio);
            ...
            if        ( m_varnames[comp] == "Er" ){
                ...
                m_all_field_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::vector_potential_fp_nodal, 0_dir, lev), lev, m_crse_ratio);
            } else if ( m_varnames[comp] == "At" ){
                m_all_field_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::vector_potential_fp_nodal, 1_dir, lev), lev, m_crse_ratio);
            } else {
