                           ...
                           const int dcomp, const int ngrow )
{
    ...
    if ( vector_field[0]->is_cell_centered() ){
        ...
        Average::ToCellCenter( mf_avg, *(vector_field[0]), dcomp  , ngrow, 0, 1 );
        Average::ToCellCenter( mf_avg, *(vector_field[1]), dcomp+1, ngrow, 0, 1 );
        Average::ToCellCenter( mf_avg, *(vector_field[2]), dcomp+2, ngrow, 0, 1 );
        ...
        if (vector_field[0]->nComp() > 1) {
            ...
            AMREX_D_TERM( Average::ToCellCenter( mf_avg, *(srcmf[0]), dcomp  , ngrow );,
                          ...
                          Average::ToCellCenter( mf_avg, *(srcmf[2]), dcomp+2, ngrow ); );
            MultiFab::Copy( mf_avg, mf_avg, dcomp+1, dcomp+2, 1, ngrow);
            Average::ToCellCenter( mf_avg, *(mf_total[1]), dcomp+1, ngrow, 0, 1 );
#else
           amrex::Abort("AverageAndPackVectorField not implemented for ncomp > 1");
#endif
        } else {
                           ...
                           const int dcomp, const int ngrow )
{
    ...
    if ( scalar_field.is_cell_centered() ){
        ...
        Average::ToCellCenter( mf_avg, scalar_field, dcomp, ngrow, 0, 1 );
    } else {
        amrex::Abort("Unknown staggering.");
    }
}
