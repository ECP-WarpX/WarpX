                           ...
                           const int dcomp, const int ngrow )
{
    ...
    Average::CoarsenAndInterpolate( mf_avg, *(vector_total[0]), dcomp  , 0, 1 );
    Average::CoarsenAndInterpolate( mf_avg, *(vector_total[1]), dcomp+1, 0, 1 );
    Average::CoarsenAndInterpolate( mf_avg, *(vector_total[2]), dcomp+2, 0, 1 );
}
                           ...
                           const int dcomp, const int ngrow )
{
    ...
    if ( scalar_total->is_cell_centered() ){
        ...
        // - Fully nodal
        Average::CoarsenAndInterpolate( mf_avg, *scalar_total, dcomp, 0, 1 );
    } else {
    ...
    int coarse_ratio, int finest_level )
{
    ...
    for (int lev=0; lev<=finest_level; lev++) {
        ...
        Average::CoarsenAndInterpolate( coarse_mf[lev], source_mf[lev], 0, 0, ncomp, IntVect(coarse_ratio) );
    }
};
