...
BTDiagnostics::InitializeFieldFunctors (int lev)
{
    ...
    for (int comp=0; comp<m_cell_center_functors_at_lev_size; comp++){
        if        ( m_cellcenter_varnames[comp] == "Ex" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Efield_aux, 0_dir, lev), lev, m_crse_ratio);
        } else if ( m_cellcenter_varnames[comp] == "Ey" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Efield_aux, 1_dir, lev), lev, m_crse_ratio);
        } else if ( m_cellcenter_varnames[comp] == "Ez" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Efield_aux, 2_dir, lev), lev, m_crse_ratio);
        } else if ( m_cellcenter_varnames[comp] == "Bx" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Bfield_aux, 0_dir, lev), lev, m_crse_ratio);
        } else if ( m_cellcenter_varnames[comp] == "By" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Bfield_aux, 1_dir, lev), lev, m_crse_ratio);
        } else if ( m_cellcenter_varnames[comp] == "Bz" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Bfield_aux, 2_dir, lev), lev, m_crse_ratio);
        } else if ( m_cellcenter_varnames[comp] == "jx" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::current_fp,0_dir, lev), lev, m_crse_ratio);
        } else if ( m_cellcenter_varnames[comp] == "jy" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::current_fp,1_dir, lev), lev, m_crse_ratio);
        } else if ( m_cellcenter_varnames[comp] == "jz" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::current_fp,2_dir, lev), lev, m_crse_ratio);
...
BTDiagnostics::UpdateVarnamesForRZopenPMD ()
{
    ...
    const int ncomp_multimodefab = warpx.m_fields.get(FieldType::Efield_aux, 0_dir, 0)->nComp();
...
BTDiagnostics::InitializeFieldFunctorsRZopenPMD (int lev)
{
    ...
    for (int comp=0; comp<m_cell_center_functors_at_lev_size; comp++){
        if        ( m_cellcenter_varnames_fields[comp] == "Er" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Efield_aux, 0_dir, lev), lev, m_crse_ratio, false, ncomp);
        } else if ( m_cellcenter_varnames_fields[comp] == "Et" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Efield_aux, 1_dir, lev), lev, m_crse_ratio, false, ncomp);
        } else if ( m_cellcenter_varnames_fields[comp] == "Ez" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Efield_aux, 2_dir, lev), lev, m_crse_ratio, false, ncomp);
        } else if ( m_cellcenter_varnames_fields[comp] == "Br" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Bfield_aux, 0_dir, lev), lev, m_crse_ratio, false, ncomp);
        } else if ( m_cellcenter_varnames_fields[comp] == "Bt" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Bfield_aux, 1_dir, lev), lev, m_crse_ratio, false, ncomp);
        } else if ( m_cellcenter_varnames_fields[comp] == "Bz" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::Bfield_aux, 2_dir, lev), lev, m_crse_ratio, false, ncomp);
        } else if ( m_cellcenter_varnames_fields[comp] == "jr" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::current_fp, 0_dir, lev), lev, m_crse_ratio, false, ncomp);
        } else if ( m_cellcenter_varnames_fields[comp] == "jt" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::current_fp, 1_dir, lev), lev, m_crse_ratio, false, ncomp);
        } else if ( m_cellcenter_varnames_fields[comp] == "jz" ){
            m_cell_center_functors[lev][comp] = std::make_unique<CellCenterFunctor>(warpx.m_fields.get(FieldType::current_fp, 2_dir, lev), lev, m_crse_ratio, false, ncomp);
