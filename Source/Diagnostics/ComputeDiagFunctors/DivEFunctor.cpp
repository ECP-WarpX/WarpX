...
DivEFunctor::operator()(amrex::MultiFab& mf_dst, const int dcomp, const int /*i_buffer*/) const
{
    ...
    // If WarpX::do_cell_centered = true (for example, with the spectral
    // solver in RZ geometry), all quantities are cell-centered instead.
    amrex::IntVect cell_type;
    if (warpx.do_cell_centered) {
        cell_type = amrex::IntVect::TheCellVector();
    } else {
        cell_type = amrex::IntVect::TheNodeVector();
    }
