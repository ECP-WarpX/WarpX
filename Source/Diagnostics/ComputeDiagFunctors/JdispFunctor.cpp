...
JdispFunctor::operator() (amrex::MultiFab& mf_dst, int dcomp, const int /*i_buffer*/) const
{
    ...
    amrex::MultiFab* mf_j = warpx.m_fields.get(FieldType::current_fp, Dir{m_dir}, m_lev);
    ...
    amrex::MultiFab* mf_curlB = warpx.m_fields.get(FieldType::hybrid_current_fp_ampere, Dir{m_dir}, m_lev);
    ...
    if (hybrid_pic_model) {
        ...
        amrex::MultiFab* mf_j_external = warpx.m_fields.get(FieldType::hybrid_current_fp_external, Dir{m_dir}, m_lev);
