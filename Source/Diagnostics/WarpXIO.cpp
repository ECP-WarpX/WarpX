...
WarpX::InitFromCheckpoint ()
{
    ...
    {
        for (int i = 0; i < 3; ++i) {
            m_fields.get(FieldType::current_fp, Dir{i}, lev)->setVal(0.0);
            m_fields.get(FieldType::Efield_fp, Dir{i}, lev)->setVal(0.0);
            m_fields.get(FieldType::Bfield_fp, Dir{i}, lev)->setVal(0.0);
        }

        if (lev > 0) {
            for (int i = 0; i < 3; ++i) {
                m_fields.get(FieldType::Efield_aux, Dir{i}, lev)->setVal(0.0);
                m_fields.get(FieldType::Bfield_aux, Dir{i}, lev)->setVal(0.0);

                m_fields.get(FieldType::current_cp, Dir{i}, lev)->setVal(0.0);
                m_fields.get(FieldType::Efield_cp, Dir{i}, lev)->setVal(0.0);
                m_fields.get(FieldType::Bfield_cp, Dir{i}, lev)->setVal(0.0);
            }
        }

        VisMF::Read(*m_fields.get(FieldType::Efield_fp, 0_dir, lev),
                    amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Ex_fp"));
        VisMF::Read(*m_fields.get(FieldType::Efield_fp, 1_dir, lev),
                    amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Ey_fp"));
        VisMF::Read(*m_fields.get(FieldType::Efield_fp, 2_dir, lev),
                    amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Ez_fp"));

        VisMF::Read(*m_fields.get(FieldType::Bfield_fp, 0_dir, lev),
                    amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Bx_fp"));
        VisMF::Read(*m_fields.get(FieldType::Bfield_fp, 1_dir, lev),
                    amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "By_fp"));
        VisMF::Read(*m_fields.get(FieldType::Bfield_fp, 2_dir, lev),
                    amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Bz_fp"));

        if (WarpX::fft_do_time_averaging)
        {
            VisMF::Read(*m_fields.get(FieldType::Efield_avg_fp, 0_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Ex_avg_fp"));
            VisMF::Read(*m_fields.get(FieldType::Efield_avg_fp, 1_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Ey_avg_fp"));
            VisMF::Read(*m_fields.get(FieldType::Efield_avg_fp, 2_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Ez_avg_fp"));

            VisMF::Read(*m_fields.get(FieldType::Bfield_avg_fp, 0_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Bx_avg_fp"));
            VisMF::Read(*m_fields.get(FieldType::Bfield_avg_fp, 1_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "By_avg_fp"));
            VisMF::Read(*m_fields.get(FieldType::Bfield_avg_fp, 2_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Bz_avg_fp"));
        }

        if (is_synchronized) {
            VisMF::Read(*m_fields.get(FieldType::current_fp, 0_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "jx_fp"));
            VisMF::Read(*m_fields.get(FieldType::current_fp, 1_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "jy_fp"));
            VisMF::Read(*m_fields.get(FieldType::current_fp, 2_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "jz_fp"));
        }

        if (lev > 0)
        {
            VisMF::Read(*m_fields.get(FieldType::Efield_cp, 0_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Ex_cp"));
            VisMF::Read(*m_fields.get(FieldType::Efield_cp, 1_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Ey_cp"));
            VisMF::Read(*m_fields.get(FieldType::Efield_cp, 2_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Ez_cp"));

            VisMF::Read(*m_fields.get(FieldType::Bfield_cp, 0_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Bx_cp"));
            VisMF::Read(*m_fields.get(FieldType::Bfield_cp, 1_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "By_cp"));
            VisMF::Read(*m_fields.get(FieldType::Bfield_cp, 2_dir, lev),
                        amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Bz_cp"));

            if (WarpX::fft_do_time_averaging)
            {
                VisMF::Read(*m_fields.get(FieldType::Efield_avg_cp, 0_dir, lev),
                            amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Ex_avg_cp"));
                VisMF::Read(*m_fields.get(FieldType::Efield_avg_cp, 1_dir, lev),
                            amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Ey_avg_cp"));
                VisMF::Read(*m_fields.get(FieldType::Efield_avg_cp, 2_dir, lev),
                            amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Ez_avg_cp"));

                VisMF::Read(*m_fields.get(FieldType::Bfield_avg_cp, 0_dir, lev),
                            amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Bx_avg_cp"));
                VisMF::Read(*m_fields.get(FieldType::Bfield_avg_cp, 1_dir, lev),
                            amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "By_avg_cp"));
                VisMF::Read(*m_fields.get(FieldType::Bfield_avg_cp, 2_dir, lev),
                            amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "Bz_avg_cp"));
            }

            if (is_synchronized) {
                VisMF::Read(*m_fields.get(FieldType::current_cp, 0_dir, lev),
                            amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "jx_cp"));
                VisMF::Read(*m_fields.get(FieldType::current_cp, 1_dir, lev),
                            amrex::MultiFabFileFullPrefix(lev, restart_chkfile, level_prefix, "jy_cp"));
                VisMF::Read(*m_fields.get(FieldType::current_cp, 2_dir, lev),
