    ...
    const amrex::IntVect& bilinear_filter_stencil_length)
{
        ...
        electromagnetic_solver_id != ElectromagneticSolverAlgo::HybridPIC)
    {
        for (int i = 0; i < AMREX_SPACEDIM; i++)
        {
            ...
            if (do_psatd_JRm) {
                // With PSATD-JRm + time averaging, particles can move during 2*dt per PIC cycle.
                if (fft_do_time_averaging){
                    dt_Rho = 2._rt*dt;
                    dt_J = 2._rt*dt;
                }
                // With PSATD-JRm but without time averaging, particles can move during dt per PIC
