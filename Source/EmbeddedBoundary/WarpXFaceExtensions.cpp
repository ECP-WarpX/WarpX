...
WarpX::InitBorrowing() {
    int idim = 0;
    for (amrex::MFIter mfi(*m_fields.get(FieldType::Bfield_fp, Dir{idim}, maxLevel())); mfi.isValid(); ++mfi) {
        amrex::Box const &box = mfi.validbox();
...
WarpX::ComputeOneWayExtensions ()
{
    ...
    for(int idim = 1; idim < AMREX_SPACEDIM; ++idim) {
        ...
        for (amrex::MFIter mfi(*m_fields.get(FieldType::Bfield_fp, Dir{idim}, maxLevel())); mfi.isValid(); ++mfi) {
            ...
            amrex::Box const &box = mfi.validbox();
            auto const &S = m_fields.get(FieldType::face_areas, Dir{idim}, maxLevel())->array(mfi);
            auto const &flag_ext_face = m_flag_ext_face[maxLevel()][idim]->array(mfi);
            ...
            auto const &S_mod = m_fields.get(FieldType::area_mod, Dir{idim}, maxLevel())->array(mfi);

            const auto &lx = m_fields.get(FieldType::edge_lengths, 0_dir, maxLevel())->array(mfi);
            const auto &ly = m_fields.get(FieldType::edge_lengths, 1_dir, maxLevel())->array(mfi);
            const auto &lz = m_fields.get(FieldType::edge_lengths, 2_dir, maxLevel())->array(mfi);
...
WarpX::ApplyBCKCorrection (const int idim)
{
    ...
    for (amrex::MFIter mfi(*m_fields.get(FieldType::Bfield_fp, Dir{idim}, maxLevel()), amrex::TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        const amrex::Array4<amrex::Real> &S =  m_fields.get(FieldType::face_areas, Dir{idim}, maxLevel())->array(mfi);
        const amrex::Array4<amrex::Real> &lx = m_fields.get(FieldType::face_areas, 0_dir, maxLevel())->array(mfi);;
        const amrex::Array4<amrex::Real> &ly = m_fields.get(FieldType::face_areas, 1_dir, maxLevel())->array(mfi);;
        const amrex::Array4<amrex::Real> &lz = m_fields.get(FieldType::face_areas, 2_dir, maxLevel())->array(mfi);;
