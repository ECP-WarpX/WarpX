...
## TODO IO
#def coarsening_points_and_weights( i, sf, sc, cr ):
#    if   ( cr==1 ):
#        numpts = 1+abs(sf-sc) #2-(2*sf*sc-sf-sc+1)
#        idxmin = i-sc*(1-sf)
#    elif ( cr>=2 )):
#        numpts = 2-sf
...
for sc in [0,1]:
    for sf in [0,1]:
        ...
        if ( sf!=sc ):
            print( '\n WARNING: sc={} not equal to sf={}, continue ...'.format( sc, sf ) )
            continue
        ...
        imin ,imax  = coarse_grid_limits( sc, sf )
        ...
        print( '\n Check interpolation points and weights for coarsening' )
        print( ' -----------------------------------------------------' )

        # Coarsening: interpolation points and weights
        ...
        # Coarsening: check conservation properties
        ...
        print( '\n Check interpolation points and weights for refinement' )
        print( ' -----------------------------------------------------' )

        # Refinement: interpolation points and weights
        for ii in range ( nf ): # index on fine grid
        ...
        # Refinement: check conservation properties
        for i in range( nc ): # index on coarse grid
            ws = 0.0
            for ii in range( nf ): # index on fine grid
                numpts,idxmin,weights = refinement_points_and_weights( ii, sc, sf, cr )
