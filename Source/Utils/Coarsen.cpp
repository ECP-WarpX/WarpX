...
void
Coarsen::CoarsenMRloop ( MultiFab& mf_dst,
                         const MultiFab& mf_src,
                         const int ncomp,
                         const IntVect ngrow,
                         const IntVect crse_ratio )
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi( mf_dst, TilingIfNotGPU() ); mfi.isValid(); ++mfi)
    {
        ...
        ParallelFor( bx, ncomp,
                     [=] AMREX_GPU_DEVICE( int i, int j, int k, int n )
                     {
                         arr_dst(i,j,k,n) = Coarsen::CoarsenMRkernel(
...
void
Coarsen::CoarsenMR ( MultiFab& mf_dst,
                     const MultiFab& mf_src,
                     const IntVect crse_ratio )
{
    BL_PROFILE( "Coarsen::CoarsenMR" );
    ...
    Coarsen::CoarsenMRloop( mf_dst, mf_src, ncomp, ngrow, crse_ratio );
}
