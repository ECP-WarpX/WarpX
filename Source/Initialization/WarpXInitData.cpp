...
WarpX::PrintMainPICparameters ()
{
  ...
  #ifdef WARPX_USE_PSATD
    if ( (do_centered==0) && (field_gathering_algo == GatheringAlgo::EnergyConserving) ){
      amrex::Print()<<"                      | - staggered mode " << "\n";
    }
    else if ( (do_centered==0) && (field_gathering_algo == GatheringAlgo::MomentumConserving) ){
    if (dims=="3"){
...
WarpX::InitPML ()
{
    ...
    if (do_pml)
    {
        ...
        pml[0] = std::make_unique<PML>(0, boxArray(0), DistributionMap(0), &Geom(0), nullptr,
                             ...
                             dt[0], nox_fft, noy_fft, noz_fft, do_centered,
        ...
        for (int lev = 1; lev <= finest_level; ++lev)
        {
            ...
            pml[lev] = std::make_unique<PML>(lev, boxArray(lev), DistributionMap(lev),
                                   ...
                                   dt[lev], nox_fft, noy_fft, noz_fft, do_centered,
                                   do_moving_window, pml_has_particles, do_pml_in_domain,
