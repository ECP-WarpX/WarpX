      ...
      m_cgeom(cgeom)
{
    ...
    const amrex::BoxArray ba_Bx = amrex::convert(ba, warpx.m_fields.get("Bfield_XX",Direction{0},0)->ixType().toIntVect());
    const amrex::BoxArray ba_By = amrex::convert(ba, warpx.m_fields.get("Bfield_XX",Direction{1},0)->ixType().toIntVect());
    const amrex::BoxArray ba_Bz = amrex::convert(ba, warpx.m_fields.get("Bfield_XX",Direction{2},0)->ixType().toIntVect());
    ...
    if (eb_enabled) {
        ...
        if (WarpX::electromagnetic_solver_id == ElectromagneticSolverAlgo::Yee ||
            ...
            auto t_pml_edge_lengths = ablastr::fields::a2m(pml_edge_lengths);
