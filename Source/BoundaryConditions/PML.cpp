      ...
      m_cgeom(cgeom)
{
    ...
    if (cgeom)
    {
        ...
        pml_E_cp[0].reset( new MultiFab( amrex::convert( cba,
            WarpX::GetInstance().getEfield_cp(0,0).ixType().toIntVect() ), cdm, 3, nge ) );
        pml_E_cp[1].reset( new MultiFab( amrex::convert( cba,
            WarpX::GetInstance().getEfield_cp(0,1).ixType().toIntVect() ), cdm, 3, nge ) );
        pml_E_cp[2].reset( new MultiFab( amrex::convert( cba,
            WarpX::GetInstance().getEfield_cp(0,2).ixType().toIntVect() ), cdm, 3, nge ) );
        pml_B_cp[0].reset( new MultiFab( amrex::convert( cba,
            WarpX::GetInstance().getBfield_cp(0,0).ixType().toIntVect() ), cdm, 2, ngb ) );
        pml_B_cp[1].reset( new MultiFab( amrex::convert( cba,
            WarpX::GetInstance().getBfield_cp(0,1).ixType().toIntVect() ), cdm, 2, ngb ) );
        pml_B_cp[2].reset( new MultiFab( amrex::convert( cba,
            WarpX::GetInstance().getBfield_cp(0,2).ixType().toIntVect() ), cdm, 2, ngb ) );
        ...
        pml_j_cp[0].reset( new MultiFab( amrex::convert( cba,
            WarpX::GetInstance().getcurrent_cp(0,0).ixType().toIntVect() ), cdm, 1, ngb ) );
        pml_j_cp[1].reset( new MultiFab( amrex::convert( cba,
            WarpX::GetInstance().getcurrent_cp(0,1).ixType().toIntVect() ), cdm, 1, ngb ) );
        pml_j_cp[2].reset( new MultiFab( amrex::convert( cba,
            WarpX::GetInstance().getcurrent_cp(0,2).ixType().toIntVect() ), cdm, 1, ngb ) );
