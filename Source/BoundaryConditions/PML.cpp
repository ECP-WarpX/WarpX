...
namespace
{
                        ...
                        const amrex::Real v_sigma, const amrex::Real th_sigma)
    {
        ...
        amrex::ParallelFor(N, [=] AMREX_GPU_DEVICE (int i) noexcept
        {
            ...
            p_sigma_cumsum[i-slo] = std::cos(th_sigma)*(fac*(offset*offset*offset)/3._rt)/v_sigma;
            if (i <= ohi+1) {
                ...
                p_sigma_star_cumsum[i-sslo] = std::cos(th_sigma)*(fac*(offset*offset*offset)/3._rt)/v_sigma;
            }
        });
    }
