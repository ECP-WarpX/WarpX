                ...
                int do_pml_in_domain)
{
    ...
    if (do_pml_in_domain){
        ...
        ablastr::utils::communication::ParallelCopy(reg, totpmlmf, 0, 0, 1, IntVect(0), IntVect(0), period);
    } else {
        ...
        if (ngr.max() > 0) {
            ...
            ablastr::utils::communication::ParallelCopy(tmpregmf, totpmlmf, 0, 0, 1, IntVect(0), ngr, period);
#ifdef AMREX_USE_OMP
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    if (do_pml_in_domain){
        ...
        ablastr::utils::communication::ParallelCopy(tmpregmf, pml,0, 0, ncp, IntVect(0), IntVect(0), period);
    }
    ablastr::utils::communication::ParallelCopy(pml, tmpregmf, 0, 0, ncp, IntVect(0), ngp, period);
}
...
PML::CopyToPML (MultiFab& pml, MultiFab& reg, const Geometry& geom)
{
  ...
  const auto& period = geom.periodicity();
    ...
    ablastr::utils::communication::ParallelCopy(pml, reg, 0, 0, 1, IntVect(0), ngp, period);
}
...
PML::FillBoundaryF (PatchType patch_type)
{
    ...
    {
        const auto& period = m_geom->periodicity();
        ablastr::utils::communication::FillBoundary(*pml_F_fp, period);
    }
    else if (patch_type == PatchType::coarse && pml_F_cp && pml_F_cp->nGrowVect().max() > 0)
    {
        const auto& period = m_cgeom->periodicity();
        ablastr::utils::communication::FillBoundary(*pml_F_cp, period);
    }
}
...
PML::FillBoundaryG (PatchType patch_type)
{
    ...
    {
        const auto& period = m_geom->periodicity();
        ablastr::utils::communication::FillBoundary(*pml_G_fp, period);
    }
    else if (patch_type == PatchType::coarse && pml_G_cp && pml_G_cp->nGrowVect().max() > 0)
    {
        const auto& period = m_cgeom->periodicity();
        ablastr::utils::communication::FillBoundary(*pml_G_cp, period);
    }
}
