...
{
    WX_PROFILE_VAR_NS("WarpXFFT::CopyDualGrid", blp_copy);
    WX_PROFILE_VAR_NS("PICSAR::FftPushEB", blp_push_eb);

    auto period_fp = geom[lev].periodicity();

    WX_PROFILE_VAR_START(blp_copy);
    ...
    WX_PROFILE_VAR_STOP(blp_copy);

    WX_PROFILE_VAR_START(blp_push_eb);
    if (Efield_fp_fft[lev][0]->local_size() == 1)
       //Only one FFT patch on this MPI
    {
        for (MFIter mfi(*Efield_fp_fft[lev][0]); mfi.isValid(); ++mfi)
    ...
    WX_PROFILE_VAR_STOP(blp_push_eb);
