 ...
 */
#include "PsatdAlgorithmArbitraryPol.H"
...
PsatdAlgorithmJLinearInTime::PsatdAlgorithmArbitraryInTime(
...
PsatdAlgorithmJLinearInTime::pushSpectralFields (SpectralFieldData& f) const
{
    ...
    for (amrex::MFIter mfi(f.fields); mfi.isValid(); ++mfi)
    {
        ...
        const amrex::Array4<const amrex::Real> a0_arr = X1_coef[mfi].array();
        const amrex::Array4<const amrex::Real> b0_arr = X2_coef[mfi].array();
        const amrex::Array4<const amrex::Real> g0_arr = X3_coef[mfi].array();
        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
        {
            ...
            fields(i,j,k,Idx.Ex) = C * Ex_old
                ...
                + I * c2 * kx * (a0 * (rho_new - 2._rt * rho_mid + rho_old) ) ;
            ...
            fields(i,j,k,Idx.Bx) = C * Bx_old
                ...
                - I * a0 * (ky * (Jz_new - 2._rt * Jz_mid + J_old) - kz * (Jy_new - 2._rt * Jy_mid - Jy_old))
            ...
            if (dive_cleaning)
            {
                ...
                fields(i,j,k,Idx.F) = C * F_old + S_ck * I * k_dot_E
                    ...
                    + ( c0 * (rho_new -2._rt * rho_mid + rho_old) + b0 * (rho_new - rho_old) - S_ck/ep0 * rho_mid);
            }
    ...
    const amrex::Real dt)
{
    ...
    for (amrex::MFIter mfi(ba, dm); mfi.isValid(); ++mfi)
    {
        ...
        const amrex::Array4<amrex::Real> c0 = c0_coef[mfi].array();
