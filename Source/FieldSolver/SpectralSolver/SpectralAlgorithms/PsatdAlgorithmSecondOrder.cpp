 ...
 */
#include "PsatdAlgorithmSecondOrder.H"
...
PsatdAlgorithmSecondOrder::pushSpectralFields (SpectralFieldData& f) const
{
    ...
    for (amrex::MFIter mfi(f.fields); mfi.isValid(); ++mfi)
    {
        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
        {
            ...
            const Complex Jx_old = (J_linear) ? fields(i,j,k,Idx.Jx_old) : fields(i,j,k,Idx.Jx_mid);
            const Complex Jy_old = (J_linear) ? fields(i,j,k,Idx.Jy_old) : fields(i,j,k,Idx.Jy_mid);
            const Complex Jz_old = (J_linear) ? fields(i,j,k,Idx.Jz_old) : fields(i,j,k,Idx.Jz_mid);
            const Complex Jx_new = (J_linear) ? fields(i,j,k,Idx.Jx_new) : fields(i,j,k,Idx.Jx_mid);
            const Complex Jy_new = (J_linear) ? fields(i,j,k,Idx.Jy_new) : fields(i,j,k,Idx.Jy_mid);
            const Complex Jz_new = (J_linear) ? fields(i,j,k,Idx.Jz_new) : fields(i,j,k,Idx.Jz_mid);
            ...
            constexpr amrex::Real inv_ep0 = 1._rt/ep0;
            ...
            const amrex::Real X4 = -inv_ep0*S_ck;

            // Shortcuts for the values of rho
            Complex rho_old, rho_new;
            rho_old = (rho_linear) ? fields(i,j,k,Idx.rho_old) : fields(i,j,k,Idx.rho_mid);
            rho_new = (rho_linear) ? fields(i,j,k,Idx.rho_new) : fields(i,j,k,Idx.rho_mid);

            if (J_constant && !update_with_rho)
            {
...
void PsatdAlgorithmSecondOrder::InitializeSpectralCoefficients (
    const SpectralKSpace& spectral_kspace,
    const amrex::DistributionMapping& dm,
    const amrex::Real dt)
{
...
void PsatdAlgorithmSecondOrder::InitializeSpectralCoefficientsAveraging (
    const SpectralKSpace& spectral_kspace,
    const amrex::DistributionMapping& dm,
    const amrex::Real dt)
{
    ...
    for (amrex::MFIter mfi(ba, dm); mfi.isValid(); ++mfi)
    {
        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
        {
            ...
            if (om_s != 0.)
            {
                X5(i,j,k) = c2/ep0*(S_ck(i,j,k)/om2_s-(1._rt-C(i,j,k))/(om4_s*dt)-0.5_rt*dt/om2_s);
            }
            else
            {
...
void PsatdAlgorithmSecondOrder::CurrentCorrection (SpectralFieldData& field_data)
{
    // Profiling
