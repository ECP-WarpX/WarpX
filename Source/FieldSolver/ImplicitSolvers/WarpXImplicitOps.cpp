...
void
WarpX::FinishElectricFieldAndApplyBCs(ablastr::fields::MultiLevelVectorField const& a_En,
                                      amrex::Real a_theta, amrex::Real a_time)
{
    using warpx::fields::FieldType;

    FinishImplicitField(m_fields.get_mr_levels_alldirs(FieldType::Efield_fp, 0), a_En, a_theta);
    ApplyEfieldBoundary(0, PatchType::fine, a_time);
    FillBoundaryE(guard_cells.ng_alloc_EB, WarpX::sync_nodal_points);
}
