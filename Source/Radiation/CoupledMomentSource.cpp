...
namespace warpx::radiation
{
        ...
        CoupledMomentCallbacks const& callbacks, CoupledMomentExchange* exchange)
    {
        ...
        auto source = [&] (amrex::MultiFab const& temperature, amrex::MultiFab const& velocity,
                           amrex::MultiFab& next, amrex::MultiFab& heat,
            ...
            heat.setVal(0);
            if (options.spatial_transport) {
                amrex::MultiFab::Copy(next, radiation, 0, 0, 4, 0);
                ...
                auto const solved = TryImplicitMomentTransport(
                    next, velocity, absorption, scattering, equilibrium, spatial_transfer, geometry,
                ...
                if (!ComputeMomentTransportIncrement(next, velocity, absorption, scattering,
            ...
            for (amrex::MFIter iterator(next); iterator.isValid(); ++iterator) {
                auto const old = radiation.const_array(iterator);
                auto const b = velocity.const_array(iterator);
                ...
                auto const output = next.array(iterator);
