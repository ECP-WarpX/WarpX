    ...
    if (electromagnetic_solver_id == ElectromagneticSolverAlgo::None ||
        ...
        if (electrostatic_solver_id != ElectrostaticSolverAlgo::None)
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                ...
                "warpx.const_dt must be specified with the electrostatic solver."
            );
        else if (electromagnetic_solver_id == ElectromagneticSolverAlgo::HybridPIC)
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                m_const_dt.has_value(),
                "warpx.const_dt must be specified with the hybrid-PIC solver."
            );
        else
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                m_const_dt.has_value(),
                "warpx.const_dt must be specified when not using a field solver."
            );
