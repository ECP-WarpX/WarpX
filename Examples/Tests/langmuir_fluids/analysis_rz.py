...
def Jr( z, r, epsilon, k0, w0, wp, t) :
    ...
    Jr_array = \
        - epsilon_0 * epsilon * m_e*c**2/e * 2*r/w0**2 * \
            np.exp( -r**2/w0**2 ) * np.sin( k0*z ) * np.cos( wp*t -wp*dt/2) * wp #phase_error = wp*dt/2
    return( Jr_array )
...
def Jz( z, r, epsilon, k0, w0, wp, t) :
    ...
    dt = t / 80 # SPECIFIC to config parameters!
    Jz_array = \
        epsilon_0 * epsilon * m_e*c**2/e * k0 * \
            np.exp( -r**2/w0**2 ) * np.cos( k0*z ) * np.cos( wp*t -wp*dt/2) * wp  #phase_error = wp*dt/2
    return( Jz_array )
