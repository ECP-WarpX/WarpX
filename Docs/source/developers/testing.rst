...
* An input file for your test, in folder `Example/...`. For the PML test, the input file is at ``Examples/pml/inputs_2d``. You can also re-use an existing input file (even better!) and pass specific parameters at runtime (see below).
* A Python script that reads simulation output and tests correctness versus theory or calibrated results. For the PML test, see ``Examples/pml/analysis_pml_yee.py``. It typically ends with Python statement ``assert( error<0.01 )``.
...
.. code-block::
   ...
   [pml_x_yee]
   buildDir = .
   inputFile = Examples/pml/inputs2d
   ...
   compileTest = 0
   doVis = 0
   analysisRoutine = Examples/pml/analysis_pml_yee.py
