...
.. warning::
   ...
   There is a caching bug in Libfabric that causes WarpX simulations to occasionally hang on Crusher on more than 1 node.
...
.. warning::
   ...
   With runs above 500 nodes on Frontier, we observed issues in ``MPI_Waitall`` calls of the kind ``OFI Poll Failed UNDELIVERABLE``.
   According to the system known issues entry `OLCFDEV-1597 <https://docs.olcf.ornl.gov/systems/crusher_quick_start_guide.html#olcfdev-1597-ofi-poll-failed-undeliverable-errors>`__, we work around this also on Crusher by setting this environment variable in job scripts:
