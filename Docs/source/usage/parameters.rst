...
* ``psatd.current_correction`` (`0` or `1`; default: `1`, with the exceptions mentioned below)
    ...
    This option is currently implemented only for the standard PSATD, Galilean PSATD, and averaged Galilean PSATD schemes, while it is not yet available for the PSATD-JRm algorithm.
...
* ``psatd.update_with_rho`` (`0` or `1`)
    ...
    comoving PSATD (``psatd.v_comoving``), time averaging (``psatd.do_time_averaging=1``), div(E) cleaning (``warpx.do_dive_cleaning=1``), and PSATD-JRm (``warpx.do_psatd_JRm=1``).
...
* ``warpx.do_psatd_JRm`` (`0` or `1`; default: `0`)
    Whether to use the PSATD-JRm algorithm, where current deposition and field update are performed multiple times within each time step. The number of sub-steps is determined by the input parameter ``warpx.do_psatd_JRm_n_depositions``. Unlike sub-cycling, field gathering is performed only once per time step, as in regular PIC cycles. When ``warpx.do_psatd_JRm = 1``, we perform linear interpolation of two distinct currents deposited at the beginning and the end of the time step, instead of using one single current deposited at half time. For simulations with strong numerical Cherenkov instability (NCI), it is recommended to use the PSATD-JRm algorithm in combination with ``psatd.do_time_averaging = 1``.

* ``warpx.do_psatd_JRm_n_depositions`` (integer)
    Number of sub-steps to use with the PSATD-JRm algorithm, when ``warpx.do_psatd_JRm = 1``.
    Note that this input parameter is not optional and must always be set in all input files where ``warpx.do_psatd_JRm = 1``. No default value is provided automatically.
