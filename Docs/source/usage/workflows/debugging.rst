...
#. If your simulation already created output data files:
   Check they look reasonable before the problem occurred; are the initial conditions of the simulation as you expected?
...
#. In case of a crash, Backtraces can be more detailed if you :ref:`re-compile <install-developers>` with debug flags: ``-DCMAKE_BUILD_TYPE=Debug`` (this will make the simulation slower) and rerun.
#. If debug builds are too costly, try instead compiling with ``-DAMReX_ASSERTIONS=ON`` to activate more checks and rerun.
#. If the problem looks like an memory violating, this could be from an invalid field or particle index access.
   Try compiling with ``-DAMReX_BOUND_CHECK=ON`` (this will make the simulation very slow), and re-run.
...
#. Consider simplifying your input options and readding more options after having found a working baseline.
