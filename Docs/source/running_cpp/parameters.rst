...
* ``<species_name>.injection_style`` (`string`)
    ...
    * ``external_file``: Inject macroparticles with properties (mass, charge, position, and momentum - :math:`\gamma \beta m c`) read from an external openPMD file.
      ...
      The ``external_file`` option is currently implemented for 2D, 3D and RZ geometries, with record components ``(x,y,z)`` for 3D, ``(x,z)`` for 2D and RZ (for the latter ``x`` represents the radial direction).
