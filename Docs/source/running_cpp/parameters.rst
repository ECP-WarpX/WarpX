...
* ``<species_name>.injection_style`` (`string`)
    ...
    * ``gaussian_beam``: Inject particle beam with gaussian distribution in
      ...
      ``<species_name>.x/y/z_rms`` (standard deviation in `x/y/z`),
      ``<species_name>.x/y/z_cut`` (optional, particles with ``abs(x-x_m) > x_cut*x_rms`` are not injected, same for y and z. Note that cutting the distribution is likely to result in a lower total charge),
