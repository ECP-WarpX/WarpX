...
* ``<species_name>.injection_style`` (`string`)
    ...
    * ``external_file``: Inject macroparticles with properties (mass, charge, position, and momentum - :math:`\gamma \beta m c`) read from an external openPMD file.
      ...
      ``<species_name>.rz_take_absolute`` (`0` or `1`) optional (default is 0) when set to 1 and WarpX is run in the RZ mode, the particles ``x`` position (which corresponds to the radial direction) that are negative are reflected to positive and those particles are injected.
      ...
      The ``external_file`` option is currently implemented for 2D, 3D and RZ geometries, with record components ``(x,y,z)`` for 3D, ``(x,z)`` for 2D and RZ.
